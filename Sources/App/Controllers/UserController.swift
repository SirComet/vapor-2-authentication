...
final class UserController {
    ...
    private var passwordAuthed: RouteBuilder { return droplet.grouped(PasswordAuthenticationMiddleware(User.self)) }
    private var tokenAuthed: RouteBuilder { return droplet.grouped(TokenAuthenticationMiddleware(User.self)) }
    private var adminAuthed: RouteBuilder { return tokenAuthed.grouped(AdminMiddleware()) }
    ...
    func addRoutes() {
        ...
        adminAuthed.get("hello") { request in // (3)
                return "You have accessed a protected route"
        }
        
        droplet.post("register", handler: register)
        
        passwordAuthed.post("login", handler: login)
        
        tokenAuthed.get("user", Int.parameter,  handler: getUser)
        tokenAuthed.get("users", handler: getAllUsers)
        tokenAuthed.get("me", handler: getLoggedInUser)
        tokenAuthed.put("user", handler: updateUser)
        tokenAuthed.post("logout", handler: logout)
